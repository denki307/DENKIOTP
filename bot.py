...
# --- CONFIGURATION ---
TOKEN = 'YOUR_BOT_TOKEN' # BotFather-kitta irunthu vanganum
OWNER_ID = 123456789     # Unga Telegram User ID
UPI_ID = "shubhamarya55555@fam" # Unga UPI ID
...
def init_db():
    ...
    cursor.execute('''CREATE TABLE IF NOT EXISTS accounts (id INTEGER PRIMARY KEY, status TEXT)''')
...
def get_stats():
    ...
    conn.close()
    return {"users": users, "accs": accs, "ords": ords}
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == 'AWAITING_AMOUNT':
        ...
        qr.save(f"qr_{update.effective_user.id}.png")
        
        caption = f"✅ **Amount: ₹{amt}**\n📲 Pay to: `{UPI_ID}`\n\n📸 Now send the **payment screenshot**."
        await update.message.reply_photo(photo=open(f"qr_{update.effective_user.id}.png", 'rb'), caption=caption, parse_mode='Markdown')
        os.remove(f"qr_{update.effective_user.id}.png")
